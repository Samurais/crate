...
@Singleton
// TODO: REMOVE THIS CHECK ONCE CrateDB IS UPDATED TO ES >= 6.0
public class ClusterNameInPathDataNodesSysCheck extends AbstractSysNodeCheck {
    ...
    private static final String DESCRIPTION = "Some path.data directories [%s] contain a folder equals to the " +
                                              "cluster name, which will not be supported in future versions of " +
                                              "CrateDB. Please migrate your data. %s%d";
     ...
     * @param pathDataPaths all paths specified in path.data
     * @param nodeDataPaths all paths specified in NodeEnvironment
     *
     * @return list of all paths that contain the cluster name as folder
     */
    @VisibleForTesting
    static List<String> comparePathDataWithNodeDataPaths(String clusterName,
                                                         List<String> pathDataPaths,
                                                         Path[] nodeDataPaths) {
        List<String> nodeDataPathList = convertNodeDataPaths(nodeDataPaths);
        List<String> result = new ArrayList<>();
        int index;
        String path;
        for (String pathDataPath : pathDataPaths) {
            path = Paths.get(pathDataPath, clusterName).toString();
            index = nodeDataPathList.indexOf(path);

            if (index != -1) {
                result.add(pathDataPath);
                nodeDataPathList.remove(index);
                continue;
            }
        }
     ...
     */
    private static List<String> convertNodeDataPaths(Path[] paths) {
